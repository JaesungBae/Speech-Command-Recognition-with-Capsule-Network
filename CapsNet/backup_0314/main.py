...
if __name__ == "__main__":
    ...
    #
    parser.add_argument('--kernel', default=13, type=int,
                        help="Convlution and Primary cpasule layer's kernel size.")
    parser.add_argument('--primary_channel', default=64, type=int,
                        help="Convlution and Primary cpasule layer's kernel size.")
    ...
    StartTime = time.time()
    trX, trY, vaX, vaY, teX, teY = DATA(args.train_with, args.test_with, args.data_path, mode=args.mode) #[sample,99,40,3]
    ...
    with tf.device('/cpu:0'):
        model, eval_model, manipulate_model = CapsNet(input_shape=trX.shape[1:],
    ...
    if args.is_training == 'TRAIN':
        ...
        #train_NoDecoder(multi_model,data=(trX,trY,vaX,vaY),save_path=save_path,args=args)
